...
sys.path.insert(
	0,
	os.path.join(
		os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "src/py"
	),
)
import reporter
...
RE_FUNCTION = re.compile(
	"\s*(static|const|extern|inline)?\s*[\w_]+\s*[\*]*\s+([\w\_]+)"
)
...
class Library:
	...
	def getSymbols(self, pattern, classifiers=None):
		if classifiers and type(classifiers) not in (list, tuple):
			classifiers = list(classifiers)
		assert isinstance(pattern, str), f"Pattern should be str, got: {type(pattern)}"
		symbols = [
			_
			for _ in self.groups
			if _.type == TYPE_SYMBOL and fnmatch.fnmatch(_.name, pattern)
		]
		if classifiers:
			symbols = [_ for _ in symbols if _.classifier in classifiers]
		return symbols
...
class Group:
	...
	def __init__(
		self, type=None, name=None, code=None, doc=None, classifier=None, start=0
	):
		self.id = Group.ID
		Group.ID += 1
		self.type = type
	...
	def __repr__(self):
		return "<Group#{5}:type={0},name={1},classifier={2},start={3}@{4}>".format(
			self.type, self.name, self.classifier, self.start, id(self), self.id
		)
...
class Parser:
	...
	def Lines(cls, text):
		...
		for line in text.split("\n"):
			...
			m = [
				...
				for m in (
					_.match(line)
					for _ in (RE_DOC_LINE, RE_DOC_START, RE_DOC_BODY, RE_DOC_END)
				)
				if m
			]
			if m:
				...
				if s:
					t, line_content = TYPE_SYMBOL, s.group(1)
				else:
					t, line_content = TYPE_DOC, m[0]
					if RE_ANNOTATION.match(line_content) or line_content == "/":
						t = line_content = None
			else:
				t, line_content = TYPE_CODE, line
			if t == TYPE_DOC and RE_DELIMITER.match(line_content):
				t = line_content = None
			if t == TYPE_DOC and line_content == "[END]":
				break
			counter += 1
			if t is not None and line_content is not None:
				yield counter, t, line_content
	...
	def Groups(cls, lines):
		...
		result = [current]
		lines = list(lines)
		for i, t, line_content in lines:
			if t == TYPE_SYMBOL:
				current = Group(
					type=TYPE_SYMBOL,
					classifier=line_content.strip(),
					start=i,
				)
				result.append(current)
			elif t == TYPE_DOC:
				if not current.code:
					current.doc.append(line_content)
				else:
					current = Group(
						type=TYPE_FILE,
						doc=[line_content],
				...
				current.code.append(line_content)
			else:
				assert None
	...
	def format(self, library):
		...
		for group in library.groups:
			...
			if group.name:
				...
				line_link = (
					'\n#### <a name="%s_%s"><span class="classifier">%s</span> `%s`</a>'
					% (group.name, group.classifier, group.classifier, group.name)
				)
				d.append(line_link)
				d.append("")
				indent_doc = "\t"
